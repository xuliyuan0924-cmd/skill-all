...
**展示清单时严格使用以下格式**（每个功能独占若干行，描述自动换行，使用 markdown 勾选框）：

---

我扫描了项目，发现以下 **N 个**可沉淀的功能，请勾选要录入功能池的项目：

- [ ] **① 功能名称**
  📁 `文件路径`
  💬 一句话说明这个功能做了什么（20 字以内）
...
| 描述 | 一句话描述 |
| 需求描述 | （前 50 字预览…） |
| preview.html | 已生成，可在浏览器打开 |

有需要修改的地方请直接说，或继续处理下一个。
...
**所有选中的功能全部处理完毕后**，汇总展示，格式如下：

---

所有功能卡片已准备好，即将提交到功能池：

- [x] **① 功能名称 A** → `features/navigation/sticky-header/`
- [x] **② 功能名称 B** → `features/forms/contact-form/`
- [x] **③ 功能名称 C** → `features/media/image-carousel/`

📌 将创建 PR 提交给 PM 审核，审核通过后自动合并。

**回复「确认」即开始推送，回复「取消」放弃本次提交。**

---

收到「确认」后，**立即自动执行**以下步骤，无需设计师再做任何操作：
